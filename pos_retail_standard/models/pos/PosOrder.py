...
class POSOrder(models.Model):
    ...
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        ...
        if context.get('pos_config_id', None):
            ...
            if context.get('reference', None):
                domain = ['|', '|', ('name', '=', context.get('reference', None)), ('pos_reference', '=', context.get('reference', None)), ('ean13', '=', context.get('reference', None))]
            _logger.info(domain)
    ...
    def add_payment(self, data):
        ...
        _logger.info('[Successfully] add_payment {}'.format(data))
        return res
    ...
    def create_from_ui(self, orders, draft=False):
        if draft:
            _logger.info('===> User [%s] save Order to draft' % self.env.user.login)
        rebuildOrders = []
        orderDuplicate_ids = []
        for o in orders:
            data = o['data']
            ...
            if 'ean13' in data:  # TODO: if users duplicate tab, and push 2 order and difference amount total, we allow save order (default odoo missed and lose orders)
                existing_orders = self.env['pos.order'].search([('ean13', '=', data['ean13'])])
                if len(existing_orders) >= 1:
                    _logger.error('[create_from_ui] Order have exist before, duplicate')
                    orderDuplicate_ids.append(self.create(self._order_fields(data)).id)
                    continue
                else:
                    rebuildOrders.append(o)
        orders = rebuildOrders
        res = super(POSOrder, self).create_from_ui(orders, draft=draft)
        _logger.info('[create_from_ui] Result: %s' % res)
        for order_value in res:
            ...
            _logger.info('[Finally save Order from POS] with new ID %s', order.id)
        return res
    ...
    def _create_order_picking(self):
        ...
        _logger.info('[Order] has picking: %s' % (hasPicking))
        return res
