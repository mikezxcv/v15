...
odoo.define('pos_retail_standard.load_models', function (require) {
                ...
                for (let i = 0; i < notifications.length; i++) {
                    ...
                    if (channel == 'sync.backend') {
                        ...
                        if (model == 'product.product') {
                            ...
                            this.pos.set('ProductRecommendations', record_ids)
                        }
                    }
                }
            }
    ...
    models.load_models([
        ...
        {
            model: 'pax.terminal',
            fields: ['ip_addr', 'protocol'],
            loaded: function (self, paxes) {
                self.pax_by_id = {};
                paxes.forEach(p => {
                    self.pax_by_id[p.id] = p
                })
            }
        },
        {
        ...
        },
        {
            ...
            loaded: function (self, loyalties) {
                if (loyalties.length > 0) {
                    ...
                    self.loyalty['rewards'] = [] // todo: supported EE version if install pos_loyalty
                } else {
                    self.loyalty = false;
                }
            }
    ...
    models.PosModel = models.PosModel.extend({
        ...
        async automaticCheckingStatusOfSession() {
            const self = this;
            const sessionIsOpened = await rpc.query({
                model: 'pos.session',
                method: 'checkSessionIsOpen',
                args: [[this.pos_session.id]]
            }, {
                shadow: true,
                timeout: 60000
            }, function (err) {
                setTimeout(_.bind(self.automaticCheckingStatusOfSession, self), 15000)
            })
            if (!sessionIsOpened) {
                this.alert_message({
                    title: _t('Warning'),
                    body: _t('Your Session State is Closed, we will close POS screen few seconds later')
                })
                ...
                setTimeout(_.bind(this.automaticCheckingStatusOfSession, this), 15000);
            }
        },
        ...
        load_server_data: function () {
            ...
            return _super_PosModel.load_server_data.apply(this, arguments).then(function () {
                ...
                self.automaticCheckingStatusOfSession()
