...
odoo.define('pos_retail_standard.order', function (require) {
    ...
    const _t = core._t;
    const MultiUnitWidget = require('pos_retail_standard.multi_unit');
    ...
    models.Order = models.Order.extend({
        ...
        build_redeem_point: function () {
            ...
            for (let i = 0; i < lines.length; i++) {
                let line = lines[i];
                let line_redeem_point = line['redeem_point'] || 0;
        ...
        async setRewardProgram(reward) {
            let loyalty = this.pos.loyalty;
            if (!loyalty.product_loyalty_id) {
                return this.pos.alert_message({
                    title: _t('Warning'),
                    body: _t('Please remove module pos_loyalty of Odoo Original or disable this feature on your POS'),
                })
            }
            ...
            if (reward['type'] == 'discount_products') {
                ...
                for (let item_index in reward['gift_product_ids']) {
                    ...
                    if (product_gift) {
                        let point_will_redeem = product_gift['lst_price'] * reward['coefficient'];
                ...
                let title = 1 / reward['coefficient'] + ' points = 1 ' + this.pos.currency['name'] + ', Customer have total Points: ' + this.pos.format_currency_no_symbol(client['pos_loyalty_point']) + ' and Total Amount of Order is: ' + this.pos.format_currency(this.get_total_with_tax());
                let startingValue = 0
                if (total_with_tax <= client['pos_loyalty_point']) {
                    startingValue = total_with_tax
                } else {
                    startingValue = client['pos_loyalty_point']
                }
                ...
                if (confirmed) {
                    ...
                    if (client['pos_loyalty_point'] < next_redeem_point) {
                        return this.pos.alert_message({
                            title: _t('Error'),
                            body: _t("It not Possible Redeem Points Bigger than Customer's Points have")
                        })
                    } else {
                        ...
                        if (next_amount >= 0) {
                            ...
                            this.pos.alert_message({
                                title: _t('Successfully'),
                                body: _t('Use: ') + this.pos.format_currency(point) + _t(' for register payment.')
                            })
                        } else {
                            return Gui.showPopup('ConfirmPopup', {
                                title: _t('Warning'),
