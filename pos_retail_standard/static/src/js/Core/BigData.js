odoo.define('pos_retail_standard.big_data', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        async getImageProducts() {
            ...
            if (!datas) {
                this.getDatasByModel('product.product', [['id', 'in', this.product_ids]], ['image_128']).then(function (datas) {
                    if (!datas) {
                        return false
                    }
                    console.log('[loaded] Image of Products: ' + datas.length)
        ...
        save_results: function (model, results) {
            ...
            if (model == 'product.product') {
                ...
                // this.setLoadingMessage(_t('Loaded total Products : ' + this.format_currency_no_symbol(this.total_products) + '( ' + process_time.toFixed(0) + ' % )'), process_time / 100);
                console.log('[save_results] model: ' + model + ' total products: ' + this.total_products)
                console.log(_t('Loaded total Products : ' + this.format_currency_no_symbol(this.total_products) + '( ' + process_time.toFixed(0) + ' % )'), process_time / 100)
            ...
            if (model == 'res.partner') {
                ...
                // this.setLoadingMessage(_t('Loaded total Partners : ' + this.format_currency_no_symbol(this.total_clients) + '( ' + process_time.toFixed(0) + ' % )'), process_time / 100);
                console.log('[save_results] model: ' + model + ' total clients: ' + this.total_clients)
                console.log(_t('Loaded total Partners : ' + this.format_currency_no_symbol(this.total_clients) + '( ' + process_time.toFixed(0) + ' % )'), process_time / 100)
        ...
        //     });
        // },
        load_server_data_from_cache: function (refeshCache = false, needLoaded = false) {
            var self = this;
            var progress = 0;
                ...
                async function load_model(index) {
                    ...
                    if (index >= self.models.length) {
                        ...
                        if (model.model) {
                            ...
                            if (!refeshCache && currentPosSessionId == odoo.pos_session_id && model.model != 'pos.config' && model.model != 'pos.session') {
                                try {
